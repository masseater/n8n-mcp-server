...
- **開始日時**:
- **TDDステップ**:
  - [ ] Red: 既存のPOST /mcpハンドラーの動作を検証する単体テストを作成
  - [ ] Green: テストが通ることを確認（リファクタリング前）
  - [ ] Refactor: ハンドラーロジックを`handleMcpRequest()`メソッドに抽出
- **依存関係**: なし
...
- **開始日時**:
- **TDDステップ**:
  - [ ] Red: ヘッダー抽出のテストケースを作成
    - ヘッダーが存在しない場合 → 400 Bad Request
    - ヘッダーが空文字の場合 → 400 Bad Request
    - ヘッダーが配列の場合 → 最初の値を使用
    - ヘッダーが正常な文字列の場合 → API Keyを返す
  - [ ] Green: src/server/mcp-server.tsにヘッダー抽出ロジックを実装
  - [ ] Refactor: エラーハンドリングを整理
- **依存関係**: タスク1
- **変更ファイル**: src/server/mcp-server.ts
...
- **開始日時**:
- **TDDステップ**:
  - [ ] ドキュメントセクション追加（HTTP Mode Authenticationセクション）
  - [ ] curlコマンドの使用例を追加
  - [ ] エラーケースの説明を追加
- **依存関係**: タスク4
- **変更ファイル**: CLAUDE.md
...
- [ ] 全タスク完了
- [ ] 全テスト通過
- [ ] 品質チェックコマンドが成功（pnpm run lint、pnpm run type-check、pnpm run build）
- [ ] コードレビュー承認（必要に応じて）
