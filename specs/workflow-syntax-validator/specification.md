...
- **ビジネスルール**:
  - **サポートするノードタイプの範囲**: 全ノードタイプ（n8n公式ノード + カスタムノード、czlonkowski/n8n-mcp実績：541ノード100%カバー）
  - **未知のノードタイプの扱い**: エラーとして扱い、類似ノード提案を表示（czlonkowski/n8n-mcp方式）
- **バリデーション**:
  - ノードタイプの存在チェック
  - 必須パラメータの存在と型チェック
  - パラメータ値の範囲・形式チェック
...
- **ビジネスルール**:
  - **条件分岐の必須出力数**: 片方のみ可（czlonkowski/n8n-mcp実装：警告レベルで提案）
  - **ループの最大深度制限**: 制限なし（czlonkowski/n8n-mcp実装：正当なループノードは循環を許可）
- **バリデーション**:
  - 分岐ノードの出力パターンチェック
  - ループ構造の完全性チェック
  - サブワークフローの整合性チェック
...
- **出力**: フォーマットされたエラーレポート
- **ビジネスルール**:
  - **エラーメッセージの言語**: 日本語（我々のMCPサーバー仕様に合わせる、czlonkowski/n8n-mcpは英語）
  - **修正提案の詳細度**: 簡潔な提案 + 具体例（czlonkowski/n8n-mcpは冗長すぎるため改善）
- **バリデーション**:
  - レポート形式の妥当性
  - メッセージの可読性チェック
...
- **レスポンスタイム**: 
  ...
  - 大規模ワークフロー（50ノード以上）: 500ms以内（czlonkowski/n8n-mcp実績値、ただしDB依存なし設計のため更に高速化可能）
- **同時処理**: 制限なし（状態を持たない関数型設計により並列処理対応）
- **メモリ使用量**: 1ワークフローあたり最大10MB（czlonkowski/n8n-mcpは68MB DB依存だが我々はDB不要設計）
...
- **ログデータ**: 既存MCPサーバーのログ設定に従う（winston使用、保存しない方針）
...
- **リソース制限**: 追加のサーバーリソースを最小限に抑制（DB不要設計で軽量化）
- **開発期間**: Phase別計画書作成後に決定
