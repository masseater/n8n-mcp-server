...
### ビジネス的制約
- **バリデーション処理の応答時間要件**: 100-500ms（czlonkowski/n8n-mcp実績値を参考）
- **サポートするワークフローの最大サイズ**: 制限なし（czlonkowski/n8n-mcp実績：大規模ワークフロー対応）
- **エラー報告の詳細度レベル**: プロファイルで調整可能（minimal/runtime/ai-friendly/strict）
